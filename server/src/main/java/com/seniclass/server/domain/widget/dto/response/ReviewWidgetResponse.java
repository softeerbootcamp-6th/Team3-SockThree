...
public record ReviewWidgetResponse(@Schema(requiredMode = Schema.RequiredMode.REQUIRED, description = "상위 리뷰 목록") List<ReviewItem> topReviews) {
    ...
    public record ReviewItem(
            @Schema(requiredMode = Schema.RequiredMode.REQUIRED, description = "리뷰 ID") Long reviewId,
            @Schema(requiredMode = Schema.RequiredMode.REQUIRED, description = "리뷰 내용") String content,
            ...
            @Schema(requiredMode = Schema.RequiredMode.REQUIRED, description = "학생 리뷰 비율") Double studentReviewRatio // 해당 학생의 전체 수강후기 작성 비율
            ) {}
}
