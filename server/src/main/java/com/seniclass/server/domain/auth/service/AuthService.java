...
public class AuthService {
  ...
  public LoginResponse login(LoginRequest request) {
    ...
    return new LoginResponse(
        ...
        new LoginResponse.UserInfo(user.getId(), user.getEmail(), user.getRole().getValue()));
  }
  ...
  public boolean isTokenValid(String token) {
    return jwtTokenProvider.validateToken(token) && !tokenManagementService.isBlacklisted(token);
  }
  ...
  public RegisterResponse registerStudent(StudentRegisterRequest request) {
    ...
    String userId =
        userAuthenticationService.createStudent(
            request.name(), request.email(), request.age(), request.gender(), encodedPassword);
