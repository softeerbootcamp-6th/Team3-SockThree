...
public record LectureCreateRequest(
        ...
        @Schema(description = "최대 수강 인원", example = "30", minimum = "1", maximum = "200")
                ...
                Integer maxStudent,
        @Schema(description = "수강료", example = "150000", minimum = "0")
                @NotNull(message = "수강료는 필수입니다.")
                @Min(value = 0, message = "수강료는 0원 이상이어야 합니다.")
                Integer fee,
        @Schema(description = "강의명", example = "왕초보 골프 스윙 마스터 클래스")
        ...
        @Schema(description = "강의 상세 설명", example = "본 강의는 초보자를 위한 맞춤형 커리큘럼으로 구성되어 있습니다.")
                ...
                String description,
        @Schema(description = "강의 시간표 목록")
                @NotNull(message = "강의 시간표는 필수입니다.")
                @NotEmpty(message = "강의 시간표는 하나 이상 등록해야 합니다.")
                List<UploadTimeCreateRequest> uploadTimeList) {
